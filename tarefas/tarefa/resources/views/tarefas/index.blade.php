...
<a href="{{ route('tarefas.create') }}" class="btn btn-success btn-sm floatend">Adicionar</a>
...
<td data-title="Descrição">{{ $row->tarefa_descricao
...
@csrf
@method('DELETE')
<a href="{{ route('tarefas.show', $row->id) }}"class="btn btn-primary btnsm">Visualizar</a>

<a href="{{ route('tarefas.edit', $row->id) }}"class="btn btn-warning btnsm">Editar</a>
...
@section('js')
<script>
$(document).ready(function() {$('#btn-search').click(function() {var searchTerm =$('#search').val().toLowerCase();

$('#tarefas-table tbody tr').each(function() {var title = $(this).find('td[data-title="Título"]').text().toLowerCase();

var description = $(this).find('td[datatitle="Descrição"]').text().toLowerCase();

if (title.includes(searchTerm) ||description.includes(searchTerm)) {$(this).show();
...
$('#btn-filter').click(function() {var statusFilter = $('#status').val();
var prioridadeFilter = $('#prioridade').val();

$('#tarefas-table tbody tr').each(function() {var status = $(this).find('td[data-title="Status"]').text();
...
if (statusMatch && prioridadeMatch) {$(this).show();
} else {
$(this).hide();
}
});
...
@else
<tr>
<td colspan="5" class="text-center">Arquivo não
